...
RSpec.describe GemSearch do
  it 'keeps name matches before info-only matches' do
    gems = RubygemsCli::Gem.build_collection([
                                               ...
                                               { 'name' => 'admin-kit', 'info' => 'works with rails', 'downloads' => 100,
                                                 'licenses' => ['MIT'] }
                                             ])

    results = described_class.new(gems, query: 'rails').results
  ...
  it 'applies license filter and downloads sorting together' do
    ...
    results = described_class.new(gems, query: 'rails', license: 'MIT', most_downloads_first: true).results
